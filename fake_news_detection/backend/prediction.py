...
from fusion.verdict_fusion import fuse_verdict
from fusion.forensic_analyzer import analyze_forensic_sources
...
def verify_claim_with_gemini(claim_text):
    """
    Fallback verification using Gemini's knowledge base when web search fails.
    """
    try:
        model = genai.GenerativeModel('gemini-2.5-flash')
        prompt = f"""
        You are an expert fact-checking AI. 
        Verify the following news claim based on your training data (up to your knowledge cutoff).
        
        CLAIM: "{claim_text}"
        
        Provide a verdict in strictly JSON format:
        {{
        ...
        If the claim is widely known as true, label REAL with high confidence.
        If it is a known debunked story or conspiracy, label FAKE.
        If unverified or ambiguous, label UNCERTAIN.
        """
        ...
        if json_match:
            ...
            return {
                ...
                'summary': data.get('explanation', 'AI Knowledge Base Verification'),
...
def create_comprehensive_prompt(evidence):
    ...
    prompt += """
        ...
        **C. External Verification:**
        ...
        Please provide a thorough, evidence-based analysis that helps users understand whythis content is classified as {evidence['prediction']}.
        """
    return prompt
...
def generate_explanation_with_gemini(evidence):
    ...
    prompt = create_comprehensive_prompt(evidence)
...
def analyze(title,image_path,model,tokenizer,classifier):
    """
    Analyze news article using multimodal fusion architecture.
    
    ARCHITECTURE:
    Stage 1: Collect all modalities (MuRIL, Image, SHAP, Grad-CAM, SERP)
    Stage 2: Fuse verdicts with forensic override
    Stage 3: Return fused evidence to frontend
    
    Args:
        title: News headline text
        image_path: Path to news image
        model: FakeNewsModel instance
        tokenizer: MuRIL tokenizer
        classifier: Zero-shot classifier
    
    Returns:
        evidence: Dict with fused verdict and all analysis results
    """
    ...
    # ========== STAGE 1: COLLECT ALL MODALITIES ==========
    print("\n" + "="*60)
    print("STAGE 1: COLLECTING MULTIMODAL EVIDENCE")
    print("="*60)
    
    # 1.1 Text + Image Model (MuRIL + CNN)
    print("\n[1/6] Running MuRIL + CNN model...")
    ...
    # Store MuRIL model output (DO NOT decide verdict yet!)
    muril_prob = fake_out.item()
    muril_label = 'FAKE' if muril_prob > 0.5 else 'REAL'
    
    muril_verdict = {
        'label': muril_label,
        'confidence': muril_prob if muril_label == 'FAKE' else (1 - muril_prob),
        'raw_score': muril_prob
    }
    
    print(f"   MuRIL: {muril_label} (confidence: {muril_verdict['confidence']:.3f})")

    # 1.2 Image Analysis (for now, same as MuRIL - can be separated later)
    image_verdict = {
        'label': muril_label,
        'confidence': muril_verdict['confidence']
    }

    # 1.3 Claim Classification
    print("\n[2/6] Classifying claim type...")
    ...
    # 1.5 Image Attention Analysis
    print("\n[4/6] Analyzing visual attention (Grad-CAM)...")
    ...
    # 1.6 FORENSIC FACT-CHECK (CRITICAL - Must complete before verdict!)
    print("\n[5/6] Running forensic fact-check (SERP)...")
    web_sources = serp_check(title)
    
    # Analyze forensic sources
    forensic_result = analyze_forensic_sources(web_sources)
    
    print(f"   Found {len(web_sources)} web sources")
    print(f"   Forensic Verdict: {forensic_result['label']} (confidence: {forensic_result['confidence']:.3f})")
    print(f"   Credibility Score: {forensic_result['credibility_score']:.3f}")
    print(f"   Credible Sources: {forensic_result['credible_source_count']}")
    print(f"   Credible Sources: {forensic_result['credible_source_count']}")
    
    # --- FALLBACK: If forensic confidence is 0, use Gemini ---
    if forensic_result['confidence'] < 0.1:
        print("\n[5.5/6] Web evidence weak. invoking Gemini Knowledge Base...")
        gemini_verdict = verify_claim_with_gemini(title)
        
        if gemini_verdict and gemini_verdict['label'] != 'UNCERTAIN':
            print(f"   Gemini Verdict: {gemini_verdict['label']} ({gemini_verdict['confidence']:.2f})")
            
            # Merge into forensic result (Simulation of web forensics via AI)
            forensic_result['label'] = gemini_verdict['label']
            forensic_result['confidence'] = gemini_verdict['confidence']
            forensic_result['forensic_summary'] = f"AI Knowledge Base: {gemini_verdict['summary']}"
            
            # Synthesize a 'source' so frontend displays something
            forensic_result['sources'].append({
                'title': 'AI Knowledge Base Verification',
                'snippet': gemini_verdict['summary'],
                'link': 'https://gemini.google.com' 
            })
            forensic_result['fake_signals'] = 1 if gemini_verdict['label'] == 'FAKE' else 0
            forensic_result['real_signals'] = 1 if gemini_verdict['label'] == 'REAL' else 0

    # ========== STAGE 2: FUSE VERDICTS ==========
    print("\n" + "="*60)
    print("STAGE 2: FUSING MULTIMODAL VERDICTS")
    print("="*60)
    
    print("\n[6/6] Applying fusion algorithm...")
    fused_result = fuse_verdict(
        muril_verdict=muril_verdict,
        image_verdict=image_verdict,
        forensic_verdict=forensic_result,
        fusion_strategy="forensic_override"
    )
    
    print(f"\n   Final Verdict: {fused_result['final_label']}")
    print(f"   Final Confidence: {fused_result['final_confidence']:.3f}")
    print(f"   Fusion Source: {fused_result['fusion_source']}")
    
    if fused_result['override_applied']:
        print(f"   ⚠️  FORENSIC OVERRIDE APPLIED!")
        print(f"       Local Model: {muril_label}")
        print(f"       Forensic: {forensic_result['label']}")
        print(f"       Final: {fused_result['final_label']}")
    
    print("\n   Fusion Reasoning:")
    for reason in fused_result['reasoning']:
        print(f"       • {reason}")

    # ========== STAGE 3: RETURN FUSED EVIDENCE ==========
    print("\n" + "="*60)
    print("STAGE 3: PREPARING EVIDENCE FOR FRONTEND")
    print("="*60)
    ...
    evidence = {
        ...
        # FUSED VERDICT (what frontend displays)
        'prediction': fused_result['final_label'],
        'confidence': fused_result['final_confidence'],
        'fusion_source': fused_result['fusion_source'],
        'override_applied': fused_result['override_applied'],
        
        # Component verdicts (for transparency)
        'component_verdicts': {
            'muril': muril_verdict,
            'image': image_verdict,
        ...
        # Fusion reasoning
        'fusion_reasoning': fused_result['reasoning']
    }
    
    print(f"\n✓ Evidence package ready for frontend")
    print(f"  Final Verdict: {evidence['prediction']} ({evidence['confidence']:.1%})")
    print("="*60 + "\n")
